...
//  Serial.println((current_value-previous_value) / previous_value);
  delayMicroseconds(100);
  if(count == maxCount){
    if(debug){
...
//      Serial.println();
    ...
    int rising_edge_threshold = 100;
    while(rising_edge_count < rising_edge_threshold){
      previous_value = current_value;
      current_value = analogRead(analogPinPhoto);
      delayMicroseconds(100);
      if(current_value > previous_value){
    ...
    no_hole_count = 0; // Reset no_hole_count
    return 1.0/((current_time-previous_time)*pow(10.0,-6)*16.0);
  }
  // rising edge: (current_value-previous_value) / previous_value > dSlope
  // falling edge: (current_value-previous_value) / previous_value < dSlope
  else if((current_value-previous_value) < dSlope && current_value > 30 && previous_value > 30){
    if(debug){
//        Serial.println(previous_value);
...
//        Serial.println(current_value);
    ...
    no_hole_count++;
    if(no_hole_count > no_hole_thresh){
      no_hole_count = 0; // Reset no_hole_count
      return 0;
    }
    return readRPSRate(count, current_value, maxCount, previous_time);
...
void loop(){
  ...
  for(int i=0; i<4; i++){
    count += readRPSRate(0, analogRead(analogPinPhoto), 1, (unsigned int)Time);
  }
  Serial.println(count/4.0);


  //Serial.println(4.0/((current_time-previous_time)*pow(10.0,-6)*16.0));
//  count += readRPSRate(0, analogRead(analogPinPhoto), 1);
//  // count+= = readRPSSign(0, analogRead(analogPinPhoto), 2, (unsigned int)Time);
